...
double dactf(double y, Act act)
{
    ...
    case ACT_SIN:  return cos(asin(y));
    }
    NN_ASSERT(0 && "Unreachable");
    return 0.0f;
}
...
Mat mat_alloc(Region *r, size_t rows, size_t cols)
{
    ...
    m.rows = rows;
    m.cols = cols;
    m.elements = reinterpret_cast<float*>(region_alloc(r, sizeof(*m.elements)*rows*cols));
...
void row_act(Row r)
{
    for (size_t j = 0; j < r.cols; ++j) {
         MAT_AT(r, 0, j) = actf(MAT_AT(r, 0, j), r.act);
    }
}
...
NN nn_alloc(Region *r, size_t *arch, size_t arch_count)
{
    ...
    nn.ws = reinterpret_cast<Mat*>(region_alloc(r, sizeof(*nn.ws)*(nn.arch_count - 1)));
    NN_ASSERT(nn.ws != NULL);
    nn.bs = reinterpret_cast<Row*>(region_alloc(r, sizeof(*nn.bs)*(nn.arch_count - 1)));
    NN_ASSERT(nn.bs != NULL);
    nn.as = reinterpret_cast<Row*>(region_alloc(r, sizeof(*nn.as)*nn.arch_count));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (size_t l = nn.arch_count-1; l > 0; --l) {
            for (size_t j = 0; j < nn.as[l].cols; ++j) {
                double a = ROW_AT(nn.as[l], j);
                double da = ROW_AT(g.as[l], j);
                //double qa = dactf(a, NN_ACT);
                double qa = dactf(a, nn.as[l].act);
                ROW_AT(g.bs[l-1], j) += s*da*qa;
                for (size_t k = 0; k < nn.as[l-1].cols; ++k) {
                    // j - weight matrix col
                    // k - weight matrix row
                    double pa = ROW_AT(nn.as[l-1], k);
                    double w = MAT_AT(nn.ws[l-1], k, j);
...
Region region_alloc_alloc(size_t capacity_bytes)
{
    ...
    r.words = reinterpret_cast<uintptr_t*>(words);
    return r;
}
...
{
    delete[] _row.elements;
}

void nn_deallocation(NN _nn)
{
    // deallocation Matrix data
    for (size_t i = 0; i < _nn.arch_count - 1; i++)
    {
        row_deallocation(_nn.as[i]);
        row_deallocation(_nn.bs[i]);
        matrix_deallocation(_nn.ws[i]);
    }
    // and last layer(output)
    row_deallocation(_nn.as[_nn.arch_count-1]);
...
Matrix::Matrix(Mat _mat)
{
    this->m_row = _mat.rows;
    this->m_col = _mat.cols;
    for (size_t i = 0; i < _mat.rows * _mat.cols; i++)
    {
        this->m_vec.push_back(_mat.elements[i]);
    }

    matrix_deallocation(_mat);
}
